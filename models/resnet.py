...
class ResUnetv4(nn.Module):
    def __init__(self, model_version, pretrained=True, num_classes=1, classification=False, in_channels=1):
        ...
        self.classification = classification  # zsx 将此处设置为false

        if "resnet18" in model_version:
            self.resnet = torchvision.models.resnet18(pretrained=pretrained)
        elif "resnet34" in model_version:
            self.resnet = torchvision.models.resnet34(pretrained=pretrained)  # zsx_此处不使用pretrained
        else:
...
def resnet_model_selector(model_name, num_classes=1, classification=False, in_channels=1):
    if "resnet" in model_name and "unet" in model_name:
        if "scratch" in model_name:
            ...
            ).cuda()  # 将模型部署到GPU上，有更好的部署方式， model.to(device)
        else:
