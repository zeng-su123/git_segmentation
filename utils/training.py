...
def reshape_masks(ndarray, to_shape):
    ...
    if h_in > h_out:  # center crop along h dimension
        ...
        # 由于对大尺寸图像做了裁切中心的调整，因此需要需要对映射回原图做一些调整上-------
        if h_out < 480 and w_out < 480:
            pad_h = (h_out - h_in)
            rem = pad_h % 2
            pad_dim_h = (math.ceil(pad_h / 2), math.ceil(pad_h / 2 + rem))
            # npad is tuple of (n_before, n_after) for each (h,w,d) dimension
            ...
            pad_h = (h_out - h_in)
            rem = pad_h % 2
            pad_dim_h = (math.ceil(pad_h / 2)-48, math.ceil(pad_h / 2 + rem)+48)
            # npad is tuple of (n_before, n_after) for each (h,w,d) dimension
            npad = (pad_dim_h, (0, 0))
            ndarray = np.pad(ndarray, npad, 'constant', constant_values=0)
        # 由于对大尺寸图像做了裁切中心的调整，因此需要需要对映射回原图做一些调整上-------
...
def plot_save_pred(original_img, original_mask, pred_mask, save_dir, img_id):
    ...
    plt.savefig(pred_filename, dpi=200, pad_inches=0.2, bbox_inches='tight')   # 不保存着
    plt.close()
...
def val_step(val_loader, model, criterion, weights_criterion, multiclass_criterion,
    ...
    with torch.no_grad():
        for sample_indx, (image, original_img, original_mask, mask, img_id) in enumerate(val_loader):
            # 通过查阅原始的 数据集迭代索引的代码 ，发现确实满足这个顺序 ，当处于验证集的时候 (image, original_img, original_mask, mask, img_id)
            # original_img ,original_mask 代表原始的img ,以及原始对应的mask，
            # image 代表预处理后的图片，shape为[1,1,224,224]
            # image_id 代表图片的id号

            original_mask = original_mask.cuda()   # 原始的mask
            image = image.type(torch.float).cuda()
            prob_pred = model(image)  # 此时的维度为[1,4,224,224]

            # ToDo: Fix val loss
            # val_loss.append(calculate_loss(mask, prob_pred, criterion, weights_criterion, multiclass_criterion).item())
            val_loss.append(0.0)  # 终于知道val_loss 为什么一直是0了，原来这里一直在+0

            for indx, single_pred in enumerate(prob_pred):

                original_mask = original_mask[indx].data.cpu().numpy().astype(np.uint8).squeeze()  # 删除维度
                ...
                for current_class in np.unique(np.concatenate((original_mask, pred_mask))):  # 此处的current_class代表的是类别，
                    # 如 背景， 左心室，左心室心肌，右心室，分别代表0 ，1，2，3。 其中背景0，每张图片都有，但是分割目标1，2，3并不一定每张图片都有
                    ...
                    # -1 Due index 0 is background   #  不考虑背景
                    if current_class != 0:
