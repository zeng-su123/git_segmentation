...
def common_test_augmentation(img_size):  # 最常规的图像增强操作，
    return [
        albumentations.PadIfNeeded(min_height=img_size, min_width=img_size, always_apply=True),  # 填充
        albumentations.CenterCrop(height=img_size, width=img_size, always_apply=True),  # 中心裁剪
        # albumentations.Crop(height=img_size, width=img_size, always_apply=True),  # 中心裁剪
        albumentations.Resize(img_size, img_size, always_apply=True)  # resize
    ]
...
def da_policy_rotate(img_size):  # DA_Rotate
    ...
    train_aug = [
        ...
        # albumentations.VerticalFlip(p=0.5),  # 添加的
        # albumentations.HorizontalFlip(p=0.5), # 添加的
    ]
...
def da_policy_combination_old(img_size):
    ...
    train_aug = [
        ...
        albumentations.HorizontalFlip(p=0.3),
        albumentations.Rotate(p=0.625, limit=45, interpolation=1, border_mode=0),
        # albumentations.RandomBrightnessContrast(p = 0.325)   # 自己添加的
    ]
