...
class MMsDataset(Dataset):
    ...
    def __init__(self, mode, transform, img_transform, train_fold=0, val_fold=0, folding_system="vendor",
        ...
        MMs_DATA_PATH = r'H:\other_files\Heart_OpenDataset\OpenDataset\Training'
        ...
        # ------------------zsx添加--------------------------------#
        # if os.path.exists(r"H:\other_files\Heart_OpenDataset\OpenDataset\uitls\data/test.csv"):
        #     data_info_path = r"H:\other_files\Heart_OpenDataset\OpenDataset\uitls\data/test.csv"
        ...
        if mode in ["test"]:
            ...
            # else:
            #     assert False, "Please set the environment variable MMsCardiac_DATA_PATH. Read the README!"
            MMs_DATA_PATH = r'H:\other_files\Heart_OpenDataset\OpenDataset\Testing'
            self.base_dir = MMs_DATA_PATH
            self.mode = mode
            self.add_depth = add_depth
            self.normalization = normalization
        ...
        if folding_system == "vendor":  # 可选 ”vendor" , “patient”，“all”
    ...
    def mask_problem(self, external_code, current_slice, current_phase, data_parent):
        ...
        mask_parent = os.path.join("Training-corrected-contours", "Labeled_npy", external_code)  # 标签位置的主目录
        mask_path = os.path.join(
            self.base_dir, mask_parent, "{}_slice{}_phase{}.npy".format(external_code, current_slice, current_phase)
        )  # 包含具体切片的路径
        mask = np.load(mask_path)  # np 加载mask

        img_path = os.path.join(
            self.base_dir, data_parent,
            external_code, "{}_slice{}_phase{}.npy".format(external_code, current_slice, current_phase)
        )  # 图像的具体路径
        image = np.load(img_path)  # 加载图像数据

        original_image = copy.deepcopy(image)  # 保存原始图片数据
        original_mask = copy.deepcopy(mask)  # 保存原始mask数据

        # ----------------调整图片尺寸-------------------------#
        list = []
        for i in self.transform.transforms[0:1]:
            list.append(i)
        list.append(albumentations.Crop(x_min=image.shape[1] // 2 - 112, y_min=image.shape[0] // 2 - 56 - 112+8,
                                        x_max=image.shape[1] // 2 + 112, y_max=image.shape[0] // 2 - 56 + 112+8,
                                        always_apply=True, p=1.0))
        for i in self.transform.transforms[2:]:
            list.append(i)
        # self.transform = albumentations.Compose(list)

        self.transform_zsx = albumentations.Compose(list)
        # ----------------调整图片尺寸-------------------------#
       ...
       # ---------调整输入图片的大小----------------#
        ...
        if self.mode == "validation":  # We don't want modify mask in validation mode
            ...
            #     # for i in self.transform.transforms[0:1]:
            #     #     list.append(i)
            #     # list.append(albumentations.Crop(x_min=image.shape[1] // 2 - 112, y_min=image.shape[0] // 2 - 56 - 112,
            #     #                                 x_max=image.shape[1] // 2 + 112, y_max=image.shape[0] // 2 - 56 + 112,
            #     #                                 always_apply=True, p=1.0))
            #     # for i in self.transform.transforms[2:]:
            #     #     list.append(i)
            #     # self.transform = albumentations.Compose(list)
            #     # self.transform.transforms[1] = albumentations.Crop(x_min=image.shape[1]//2-112, y_min=image.shape[0]//2 -56-112, x_max=image.shape[1]//2+112, y_max=image.shape[0]//2 -56+112, always_apply=True, p=1.0),  # 中心裁剪
            #
            #     image, mask = apply_augmentations(image, self.transform_zsx, self.img_transform, mask)
        ...
        # -------ZSX增加-------------------
        if self.mode == "test" or self.get_id:  # We don't want modify mask in validation mode
            img_id = "{}_slice{}_phase{}".format(external_code, current_slice, current_phase)
            return [image, original_image, original_mask, mask, img_id]
        # -------ZSX增加-------------------
...
def dataset_selector(train_aug, train_aug_img, val_aug, args):
    if args.dataset == "mnms":
        train_dataset = MMsDataset(
            mode="train", transform=val_aug, img_transform=train_aug_img,
