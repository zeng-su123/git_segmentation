...
if __name__ == "__main__":
    ...
    MMs_DATA_PATH = r'H:\other_files\Heart_OpenDataset\OpenDataset'
    data_info_path = os.path.join(MMs_DATA_PATH, "Training-corrected", "M_Ms_Dataset_Information.xlsx")
    ...
    for index, row in data_info_df.iterrows():
        ...
        if os.path.exists(os.path.join(r'H:\other_files\Heart_OpenDataset\OpenDataset\Validation',
                                       row["External code"])):
            data_info_df.at[index, 'Labeled'] = True
            numbers = numbers + 1
        # elif not os.path.exists(os.path.join(r'H:\other_files\Heart_OpenDataset\OpenDataset\Training', "Unlabeled", row["External code"])):
    ...
    #             r'H:\other_files\Heart_OpenDataset\OpenDataset\Training', "Labeled",
    ...
    #
    # plt.savefig(r"H:\train_meta_cases.png", dpi=150)
    # print("Done!")
    ...
    for index, row in data_info_df.iterrows():
        ...
        length = len(data_info_df)
        # z = data_info_df.loc[len(data_info_df)]
        data_info_df.loc[len(data_info_df)] = {
            "External code": row["External code"],"VendorName": row["VendorName"] ,"Vendor": row["Vendor"],
    ...
    data_info_df = data_info_df[data_info_df["Labeled"] == True]
    ...
    for index, row in data_info_df.iterrows():
        ...
        # img_example = os.path.join(
        #     r'H:\other_files\Heart_OpenDataset\OpenDataset\Training', "Labeled" if row["Labeled"] else "Unlabeled",
        #     img_example, "{}_sa.nii.gz".format(img_example)
        # )
        img_example = os.path.join(
            r'H:\other_files\Heart_OpenDataset\OpenDataset\Validation',
            img_example, "{}_sa.nii.gz".format(img_example)
        )
        zsx_test = load_nii(img_example)
    ...
    os.makedirs(r"H:\other_files\Heart_OpenDataset\OpenDataset\uitls\data", exist_ok=True)
    # data_info_df.to_csv("utils/data/train.csv", index=False)
    data_info_df.to_csv(r"H:\other_files\Heart_OpenDataset\OpenDataset\uitls\data/validation.csv", index=False)
